...
def save_data(xx, cc_scaled_best, cc_scaled_means, ccRes, tt, errors, t_best,
    ...
    ps.figure_df_profiles(xx_dummy, xlabels, cc_scaled_best, ccRes, tt, t_newX_coords,
                          D_mean, F_mean, D_std, F_std, plt_profiles=15, save=True,
                          savePath=savePath)
...
def average_data(result, xx, cc, top_percent=1):
    ...
    # NOTE: prepending DSol values here
    averages, stdevs = np.insert(averages, 0, DSol), np.insert(stdevs, 0, 0)
    best_results = np.insert(best_results, 0, DSol)
...
def initialize_optimization(runs, params, n_profiles, xx, DMax=1000, FMax=20):
    ...
    # set D, F bounds, NOTE: changing bounds as DSol is fixed
    ...
    d_init = np.random.rand(runs, 1)*DMax  # randomly choose D , NOTE: only for Dmuc
...
def resFun(parameters, xx, cc, tt, dxx_dist, dxx_width, check=False):
    ...
    d = [DSol, parameters[0]]  # NOTE: DSol is fixed
    f = parameters[1:3]
...
def main():
    ...
    # set up optimization
    params = 2  # only fit here Fsol and Dmuc, Fmuc
