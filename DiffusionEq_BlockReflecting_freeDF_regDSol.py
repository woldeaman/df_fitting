...
def resFun(df, cc, xx, tt, deltaX=1, dx_dist=None, dx_width=None,
    ...
    d_og = df[:N]
    f_og = df[N:]  # letting F completely free

    # for extrapolation into bulk assume same d and f
    d = np.concatenate((np.ones(6)*d_og[0], d_og))
    f = np.concatenate((np.ones(6)*f_og[0], f_og))
    ...
    d_mean = np.mean(d_og[:18])
    regularization = alpha*(d_mean-DSol)  # regularization term only for DSol
    RRn = np.append(RRn, regularization)  # appended to residual vector
