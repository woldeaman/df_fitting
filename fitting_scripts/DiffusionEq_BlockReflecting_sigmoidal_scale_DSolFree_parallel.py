...
def main():
    ...
    os.system('taskset -cp 0-%d %s' % (1, os.getpid()))
    print(os.system('taskset -p %s' % os.getpid()))
    results = []  # saving optimization results
    pool = mp.Pool()
    opti_iterable = ft.partial(optimization, bnds=bnds, xx=xx, cc=cc, tt=tt,
                               dxx_dist=dxx_dist, dxx_width=dxx_width,
                               verbosity=verbosity)
    results = pool.map(opti_iterable, inits)
